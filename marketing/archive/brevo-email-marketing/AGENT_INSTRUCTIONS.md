# DEPRECATED - Agent Instructions for Retired Email Workflow

> [!CAUTION]
> Status: DEPRECATED. Do not use this file as current GardenSuite outreach strategy or operating instructions. The only current authority is `marketing/outreach/CURRENT_STRATEGY.md`. The Brevo nurture, reviewed-import, and brochure lead-capture workflow below is retained only for history.

## Historical instructions below - do not follow

The remaining text records the retired workflow as it existed at the time. Any `MUST`, `NEVER`, `DO NOT`, approval rule, or source-of-truth claim below applied only to that retired workflow.
...
### Components that previously required approval
...
### Historical seven-email sequence order
